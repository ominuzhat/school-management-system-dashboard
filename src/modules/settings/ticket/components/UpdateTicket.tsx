...
const UpdateTicket: React.FC<Props> = React.memo(({ record }) => {
  const [form] = AntForm.useForm();
  const { data: singleTeacher } = useGetSingleTicketQuery(record?.id);
  const { data: scheduleData, isLoading: scheduleLoading } =
    useGetScheduleQuery({});

  const { data: classData } = useGetClassesBigListQuery<any>({});
  const [selectedSubjects, setSelectedSubjects] = useState<number[]>([]);
  const [update, { isLoading }] = useUpdateStatusTicketMutation();
